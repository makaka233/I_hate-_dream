...
> 当前提醒：项目处于 **V2 后期**，当前目标是**先完成 V2**（默认参数稳定、多 seed 验证、多负载与消融完成）；**V3** 是后续方法升级目标，不是当前前置条件。
...
进一步扩展到 10 个未见 seed（`1007` 到 `1907`，步长 `100`）后，观察到一个更细的结论：

- 若以**均值最优**为目标，`wmd_if_gain` 回退略优：
  - `dual_agentd_guarded_wms`：均值约 `90.3411`
  - `keep_previous_wms`：均值约 `91.3137`
  - 但胜率约 `0.50`，跨 seed 波动更大
- 若以**稳健性**为目标，`keep_previous` 回退更适合作为默认配置：
  - `dual_agentd_guarded_wms`：均值约 `90.4618`
  - 相比 `keep_previous_wms` 平均改善约 `0.8519`
  - 胜率约 `0.90`
  - 标准差约 `10.1035`，优于 `wmd_if_gain` 的约 `11.3498`

因此，当前 V2 阶段采用的工程默认值调整为：
...
- 慢尺度守门默认：
  ...
  - `agentd_base_gain = 0.6`
  - `agentd_max_wmd_gap = 0.75`
  - `agentd_fallback_mode = keep_previous`
- 快尺度守门默认：
  - `agent_s_min_prob = 0.0`
  - `agent_s_min_margin = 0.05`
  - `agent_s_fallback = greedy`

同时保留 `wmd_if_gain + fallback_gain = 0.6` 作为**均值优化版备选配置**。

进一步在一组**未参与参数收敛的 holdout 12 个 unseen seed**（`2007` 到 `3107`，步长 `100`）上做门槛验证后：

- 采用当前稳健版默认参数时：
  - `dual_agentd_guarded_wms`：均值约 `93.4640`
  - `keep_previous_wms`：均值约 `93.9241`
  - `dual_wmd_wms`：均值约 `90.0546`
  - `win_rate_vs_keep_previous`：约 `0.9167`
- 这说明当前稳健版默认参数在新的 holdout seed 上依然成立，已经具备进入 V2 大规模测试阶段的条件
- 同时观察到：
  - `dual_agentd_wms`：均值约 `92.3616`
  - 说明未守门 Agent-D 在部分新 seed 上有更高均值潜力
  - 但 guarded 版本对 `keep_previous` 的胜率更高，更适合作为 V2 主线默认配置

当前判断：

- **V2 主线默认配置**：使用 guarded Agent-D + guarded Agent-S
...
- 当前项目处于 **V2 后期**，不是早期原型阶段。
- 当前的 V2 已经具备完整双时间尺度闭环：
  - 慢尺度：`WM-D + Agent-D + guarded fallback`
  - 快尺度：`WM-S + Agent-S distillation + guarded fallback`
  - 双智能体联动、多 seed 校准与系统级评估脚本
- **V2 的完成态** 是：
  - 默认双智能体参数稳定
  - 更多 unseen seed 验证通过
  - 多负载实验完成
  - 消融实验完成
  - 图表、结果和论文材料整理完成
- **V3** 是方法学上的下一代目标形态，不是当前阶段必须先完成的前置条件。
- **V3 的核心特征** 是把慢尺度从“候选部署选择”升级到“候选 + 局部编辑”或“编辑式部署策略”。
- 当前提醒：**先完成 V2，再决定是否进入 V3。**
